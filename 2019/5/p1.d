...
void opAdd(int arg1, int arg2, ref int outpos)
{
    outpos = arg1 + arg2;
}

void opMul(int arg1, int arg2, ref int outpos)
{
    outpos = arg1 * arg2;
}
...
void main()
{
    ...
    validate(mem);
    // TODO randomized iteration over the cross product?
    foreach (noun, verb; cartesianProduct(iota(0, 99), iota(0, 99)))
    {
        auto temp_mem = mem.dup;
        temp_mem[1] = noun;
        temp_mem[2] = verb;
        compute(temp_mem);
        if (temp_mem[0] == 19690720)
        {
            auto result = noun * 100 + verb;
            writeln(result);
            return;
        }
    }
